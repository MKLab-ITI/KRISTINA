...
public class ContextServiceV2 {
	...
	public Response reviewDemoV2(@FormParam("emotions") String emotions, @FormParam("frames") String frames,
			@FormParam("username") String username, @FormParam("scenario") String scenario,
			@FormParam("file") String file, @FormParam("lang") String lang) {
		// return QA(emotions, frames, username, scenario, file, true);
		Response r = rules(emotions, frames, username, scenario, lang, null);

		InputStream stream = new ByteArrayInputStream(r.getEntity().toString().getBytes(StandardCharsets.UTF_8));
	...
	public Response getText(@FormParam("emotions") String emotions, @FormParam("frames") String frames,
			@FormParam("username") String username, @FormParam("scenario") String scenario,
			@FormParam("file") String file, @FormParam("lang") String lang, @FormParam("json") String json) {
		// return QA(emotions, frames, username, scenario, file, true);
		return rules(emotions, frames, username, scenario, lang, json);

	}

	private Response rules(String emotions, String frames, String username, String scenario, String lang, String json) {
		...
		// return errorResponse(ErrorHandling.error("Json cannot be empty",
		// null));
		// }
		if (Strings.isNullOrEmpty(lang)) {
			return errorResponse(ErrorHandling.error("Language cannot be empty", null));
		}
		try {
			// AdminBean.setScenario(scenario);
		} catch (Exception e) {
			// return errorResponse(ErrorHandling.error("error setting
			// scenario", e));
		}
		...
		try {
			...
			// OntModel topicModel =
			// topicBean.loadExtractedEntityResources(laResult.getDomainKeyEntities());
		...
		try {
			rulesBean.start(laResult, json, username, scenario);
			rulesBean.flashModel();
		} catch (Exception e) {
			return errorResponse(ErrorHandling.error(e.getMessage(), e));
		}

		System.out.println(Utils.modelToString(rulesBean.responseModel, "TTL"));
		return successResponse(Utils.modelToString(rulesBean.responseModel, "TTL"));
	...
	@Produces(MediaType.TEXT_PLAIN + ";charset=utf-8")
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	public Response getTextFAKE(@FormParam("data") String data, @FormParam("request") String request,
			@FormParam("user") String user, @FormParam("lang") String lang, @FormParam("userText") String userText) throws NamingException, IOException {

		LAParserResult laResult = laParserBean.extractKeyEntityTypesFAKE(data, userText, lang, request);
		try {
			rulesBean.start(laResult, null, user, "eat_sleep");
			rulesBean.flashModel();
		} catch (Exception e) {
			return errorResponse(ErrorHandling.error(e.getMessage(), e));
		}
	...
	// private Response QA(String emotions, String frames, String username,
	// String scenario, String file, boolean update) {
	// System.out.println("frames:" + frames);
	// System.out.println("emotions:" + emotions);
	// // System.out.println("username:" + username);
	// System.out.println("scenario:" + scenario);
	...
	// return errorResponse(ErrorHandling.error("Scenario cannot be empty",
	// null));
	// }
	//
	// try {
	...
	// Iterator<Entry<OntResource, Collection<Resource>>> iterator =
	// topics.entrySet().iterator();
	...
	// return errorResponse(ErrorHandling.error("Error extracting namespaces",
	// e));
	// }
	// }
	// }
	...
	// DataSourceBean.getDataSources_str(topicSet.stream().findFirst());
	...
	//
	// // newspaper
	// if (laResult.speechActType.toString().equals(Namespaces.LA_DIALOGUE +
	// "RequestNewspaper")) {
	// InitialContext jndi = new InitialContext();
	// NewspaperBean newspaperService = (NewspaperBean) jndi
	// .lookup("java:app/kristina-j2ee-ejb/NewspaperBean");
	// String content = newspaperService.start(laResult.userText);
	//
	// // need to add the topic if it has not been detected
	// Resource newspaperResource =
	// ResourceFactory.createResource(Namespaces.KB_CONTEXT +
	// "NewspaperContext");
	// if (!topicSet.contains(newspaperResource)) {
	// topicSet.add(newspaperResource);
	// }
	//
	// currentContextBean.update(topicInstance, topicSet, la_decode,
	// laResult.speechActType);
	...
	// // weather
	// } else if
	// (laResult.speechActType.toString().equals(Namespaces.LA_DIALOGUE +
	// "RequestWeather")) {
	...
	// ResourceFactory.createResource(Namespaces.KB_CONTEXT + "WeatherContext");
	...
	// laResult.speechActType);
	//
	// WeatherResponseGenerator g = new
	// WeatherResponseGenerator(currentContextBean, weatherData);
	// responseModel.add(g.generate());
	// } else if (AdminBean.getScenario().equals("pain") ||
	// AdminBean.getScenario().equals("babycare")) {
	...
	//
	// String content = PassageRetrievalBean.start(laResult.userText);
	// currentContextBean.update(topicInstance, topicSet, la_decode,
	// laResult.speechActType);
	// if (content != null) {
	// StatementResponseGenerator g = new
	...
	// UnknownResponseGenerator(currentContextBean, "structured");
	// responseModel.add(g.generate());
	// }
	// }
	//
	...
	// // AdditionalResponseGenerator g = new
	// // AdditionalResponseGenerator(currentContextBean,
	// // "structured");
	// // responseModel.add(g.generate());
	//
	// UnknownResponseGenerator g = new
	...
	// if
	// (topicSet.contains(ResourceFactory.createResource(Namespaces.KB_CONTEXT +
	// "NewspaperContext"))) {
	// System.out.println("newpaper context has been detected without
	// dialogue");
	// UnknownResponseGenerator g = new
	// UnknownResponseGenerator(currentContextBean, "structured");
	// responseModel.add(g.generate());
	...
	// else if (topicSet
	// .contains(ResourceFactory.createResource(Namespaces.KB_CONTEXT +
	// "WeatherContext"))) {
	...
	// result.addAll(qaBean2.start(laResult.getDomainKeyEntities(), topicSet,
	// hb));
	//
	// if (result.isEmpty()) {
	// UnknownResponseGenerator g = new
	...
	// result.addAll(qaBean.start(laResult.getDomainKeyEntities(), topicSet,
	// hb));
	//
	// if (result.isEmpty()) {
	// UnknownResponseGenerator g = new
	...
	// @Produces(MediaType.TEXT_PLAIN + ";charset=utf-8")
	// @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	// public Response getText2(@FormParam("emotions") String emotions,
	// @FormParam("frames") String frames,
	// @FormParam("username") String username, @FormParam("scenario") String
	// scenario,
