...
public class LocalEventBean {
	...
	/*
	 * 
	 * German event retrieval:
URL: http://160.40.51.32:9000/ tubingenEventSearch
Response type: JSON 

	 */
	
	
	public LocalEventBean() {
	...
	public String start(String location, String lang) throws Exception {
		...
		if(lang!= null && lang.toLowerCase().equals("de") && location.equals("Tübingen")) {
			return "http://www.tagblatt.de/Veranstaltungen/Heute.html?Zeitraum=Heute&kPageId=331";
		}

		CloseableHttpClient httpclient = HttpClients.createDefault();
		URI uri = new URIBuilder().setScheme("http").setHost("160.40.51.32").setPort(9000).setPath("/getGenericEvent")
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() != 200) {
				throw new Exception(
						"Local Event service is not reachable: " + response.getStatusLine().getStatusCode());
			}
	...
	public static void main(String[] args) throws Exception {
		LocalEventBean PassageRetrievalBean = new LocalEventBean();
