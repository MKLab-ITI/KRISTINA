 ...
 *    "%JAVA_HOME%\\bin\\javac.exe" UnitexJniHandler.java
 *    "%JAVA_HOME%\\bin\\java.exe" -classpath .; UnitexJniHandler %1 %2 %3 %4
 *
 *  Under Unix (Linux, MacOSX...), a script startUnitexJniDemo.sh with these line (without the *)
 *    #!/bin/bash
 *
 *    javac UnitexJniHandler.java
 *    java -Djava.library.path=. UnitexJniHandler $1 $2 $3 $4
 *
 *  copy the JNI Library (UnitexJni.dll for MS-Windows, libUnitexJni.so for Linux, libUnitexJni.dylib for MacOSX)
 *    on the directory with UnitexJniHandler.java and the batch/script
 *
...
public class UnitexJniHandler {


        //------------------  IMPORTANT NOTE --------------------------------------
        //UnitexHniHandler needs "UnitexJni.dll" in order to work properly
        //dll is imported in class "UnitexJni", in package "fr.umlv.unitex.jni"
        //dll is stored in parent dir of the project
        //in case of artifact, dll Ηas to accompany the .jar in the same dir ALWAYS
        //-------------------------------------------------------------------------
      ...
      public static String processUnitexWork(String othersResDir, List<String> workingDicoFileNames, String workingGraphFileName, String corpusPath, String corpusText)
      {
          String pSep = pathSeparator;
          UnitexJni.writeUnitexFile(UnitexJni.combineUnitexFileComponent(corpusPath,"output.txt"),corpusText);
		  
		  // we create offsets file offset1.txt and offset2.txt to get position against the original corpus in the xml file

          String cmdNorm = "Normalize " + UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"output.txt") + " -r "+UnitexJni.combineUnitexFileComponentWithQuote(othersResDir,"Norm.txt") + " --output_offsets="+ UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"offset1.txt") ;
          String cmdTok = "Tokenize " + UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"output.txt") + " -a "+ UnitexJni.combineUnitexFileComponentWithQuote(othersResDir,"Alphabet.txt") + " --input_offsets="+ UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"offset1.txt") + " --output_offsets="+ UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"offset2.txt") ;

          String cmdDico = "";
          String dicoPrefix = "Dico -t "+ UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"output.snt")+ " -a " + UnitexJni.combineUnitexFileComponentWithQuote(othersResDir,"Alphabet.txt")+" ";
          for (String workingDicoFileName: workingDicoFileNames){
              cmdDico += dicoPrefix + UnitexJni.combineUnitexFileComponentWithQuote(workingDicoFileName);
              dicoPrefix = " ";
          }


          String cmdLocate = "Locate -t "+UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"output.snt")+ " " + UnitexJni.combineUnitexFileComponentWithQuote(workingGraphFileName)+ " -a " + UnitexJni.combineUnitexFileComponentWithQuote(othersResDir,"Alphabet.txt")+ " -L -M --all -b -Y -o 1000000";
          String cmdConcord = "Concord "+ UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"output_snt","concord.ind")+ " -m " + UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"output.txt") + " --uima="+ UnitexJni.combineUnitexFileComponentWithQuote(corpusPath,"offset2.txt") ;
          ...
          String response = UnitexJni.getUnitexFileString(UnitexJni.combineUnitexFileComponent(corpusPath,"output.txt"));
          return response;
      }
    ...
    public static void main(String [] args) {
        ...
        System.out.println("Usage : UnitexJniHandler [ressource_dir] [base_work_dir] [nb_loop] [param]");
        ...
        String baseWorkDir = UnitexJni.isUnderWindows() ? ".\\unitexFiles\\baseWork": "./unitexFiles/baseWork";
        String ressourceDir = UnitexJni.isUnderWindows() ? ".\\unitexFiles\\resources": "./unitexFiles/resources";
        int nbLoop=1;
        int cfgParam = 0;
        ...
        String graphFileName = UnitexJni.combineUnitexFileComponent(graphResDir,"master.fst2");
        ...
        String CorpusWorkPath = baseWorkDir;
        ...
        for (int i=0;i<nbLoop;i++)
        {
            ...
            String text;
            try {
                scanner = new Scanner( new File(".\\unitexFiles\\baseWork\\input.txt"), "UTF-8" );
			...
			res=processUnitexWork(othersResDir,Arrays.asList(workingDicoFileName),workingGraphFileName,CorpusWorkPath, text);
        }
