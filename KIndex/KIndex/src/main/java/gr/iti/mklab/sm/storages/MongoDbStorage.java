...
public class MongoDbStorage implements Storage {
    ...
    private static String AUTH_METHOD = "mongodb.authenticationMethod";
    ...
    public MongoDbStorage(Configuration config) {
        ...
        this.authenticationMethod = config.getParameter(MongoDbStorage.AUTH_METHOD);
    }
...
//                MorphiaManager.setup(host);
                if(authenticationMethod.equals("SCRAM-SHA-1"))
                    MorphiaManager.setup(authenticationMethod, host, database, username, password, 27017);
                else
                    MorphiaManager.setup(host);
