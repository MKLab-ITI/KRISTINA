...
public class MorphiaManager {
	...
	private MorphiaManager(String mechanismType, String host, String dbName, String username, String password, int port){
		try {
			if (mongoClient == null){
				if(mechanismType.equalsIgnoreCase("MONGODB-CR")){
					MongoCredential credential = MongoCredential.createMongoCRCredential(username, dbName, password.toCharArray());
					mongoClient = new MongoClient(new ServerAddress(host + ":" + port), Arrays.asList(credential));
				}
				if(mechanismType.equalsIgnoreCase("SCRAM-SHA-1")){
	...
	public synchronized static void setup(String authMechanism, String host, String dbName, String username, String password, int port) {
		if (instance == null){
				instance = new MorphiaManager(authMechanism, host, dbName, username, password, port);		
		}
	}
